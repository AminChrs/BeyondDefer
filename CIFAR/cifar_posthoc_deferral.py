...
def metrics_print_joint(net,expert_fn, n_classes, loader):
...
def train_reject_joint(train_loader, model, optimizer, scheduler, epoch, expert_fn, n_classes):
    """Train for one epoch on the training set with deferral"""
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
...
## Run Reject functions
def run_reject(model, n_dataset, expert_fn, epochs, alpha, train_loader, val_loader, cost, cost_list = [1] * 10):
    '''
    ...
    for epoch in range(0, epochs):
        # train for one epoch
        train_reject(train_loader, model, optimizer, scheduler, epoch, expert_fn, n_dataset, alpha, cost, cost_list)
        if epoch % 10 == 0:
...
def run_reject_joint(model, n_dataset, expert_fn, epochs, train_loader, val_loader):
    '''
    model: WideResNet model
    data_aug: boolean to use data augmentation in training
    n_dataset: number of classes
    ...
    for epoch in range(0, epochs):
        # train for one epoch
        train_reject_joint(train_loader, model, optimizer, scheduler, epoch, expert_fn, n_dataset)
        if epoch % 10 == 0:
            metrics_print_joint(model, expert_fn, n_dataset, val_loader)



def run_reject_human (model, expert_fn, epochs, train_loader, val_loader, k_list):
    '''
    only train classifier
    model: WideResNet model
    epochs: number of epochs to train
    ...
    for epoch in range(0, epochs):
        # train for one epoch
        train_reject_human(train_loader, model, optimizer, scheduler, epoch, expert_fn)
        if epoch % 10 == 0:
            metrics_print_human(model, val_loader, expert_fn, k_list)
...
def main():
    ...
    if (not is_defer):
        ...
        run_reject_class(model_ai, 100, train_loader, val_loader)
        ...
        ## joint
        print(f"\n\n----------- Training the joint classifier ----------\n\n")
        model_joint = JointNet(10, WideResNet(10, n_dataset + 1, 4, dropRate=0))
        run_reject_joint(model_joint, 10, expert.predict, 200, train_loader, val_loader)

        print("\n\n----------- Printing the metrics of the joint classifier ----------\n\n")
        accuracy_joint, class_accuracies_joint = metrics_print_joint(model_joint, expert.predict, n_dataset, val_loader)
        results['joint'] = {'accuracy': accuracy_joint, 'class accuracies': class_accuracies_joint}

        ## human
        print(f"\n\n----------- Training the human predictor ----------\n\n")
        model_human = WideResNet(10, n_dataset, 4, dropRate=0).to(device) 
        run_reject_human(model_human, expert.predict, 100, train_loader, val_loader, k_list)
        ...
        total_accuracy_hybrid, class_accuracies_hybrid, cov, cov_class = metrics_print_posthoc(model_human, model_ai, model_joint, val_loader, expert.predict, cost)
