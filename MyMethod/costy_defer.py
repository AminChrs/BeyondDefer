        ...
        with torch.no_grad():
            for batch, (data_x, data_y, hum_preds) in enumerate(dataloader):
                ...
                for i in range(len(data_y)):
                    ...
                    if self.is_defer(outputs_expert[i, 1], outputs_class[i, :], loss_fn):
                        defers.extend([1])
                    else:
                        defers.extend([0])
    ...
    def fit(
        ...
        for epoch in tqdm(range(epochs)):
            ...
            if epoch % test_interval == 0 and epoch > 1:
                ...
                val_metrics = compute_deferral_metrics_cost_sensitive(data_test, loss_fn)
        ...
        return compute_deferral_metrics_cost_sensitive(self.test(dataloader_test, loss_fn), loss_fn)
    
    def is_defer(self, outputs_expert, outputs_class, loss_fn):
...
class OVASurrogateCostSensitive(OVASurrogate):
    def __init__(self, alpha, plotting_interval, model, device, learnable_threshold_rej = False):
        super().__init__(alpha, plotting_interval, model, device, learnable_threshold_rej)
    ...
    def test(self, dataloader, loss_fn):
        ...
        with torch.no_grad():
            for batch, (data_x, data_y, hum_preds) in enumerate(dataloader):
                ...
                defer_binary = [self.is_defer(outputs.data[i], loss_fn) for i in range(len(outputs.data))]
        ...
        for epoch in tqdm(range(epochs)):
            ...
            if epoch % test_interval == 0 and epoch > 1 :
                ...
                if val_metrics["system_acc"] >= best_acc:
                    ...
                    best_model = copy.deepcopy(self.model.state_dict())
                if verbose:
                    logging.info(compute_deferral_metrics_cost_sensitive(data_test, loss_fn))
            if scheduler is not None:
                scheduler.step()
...
class LceSurrogateCostSensitive(LceSurrogate):
    ...
    def fit_hyperparam(
        ...
        for alpha in tqdm(alpha_grid):
            ...
            accuracy = compute_deferral_metrics_cost_sensitive(self.test(dataloader_val, loss_fn), loss_fn)["system_acc"]
        ...
        test_metrics = compute_deferral_metrics_cost_sensitive(self.test(dataloader_test, loss_fn), loss_fn)
        return test_metrics
