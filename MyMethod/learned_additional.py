...
class LearnedAdditional(BaseMethod):
        ...
        with torch.no_grad():
            for batch, (data_x, data_y, hum_preds) in enumerate(dataloader):
                ...
                outputs_classifier = F.sigmoid(self.model_classifier(data_x)
                                               [:, :n_classes])
                outputs_classifier /= outputs_classifier.sum(dim=1,
                                                             keepdim=True)
                outputs_meta = F.sigmoid(self.model_meta(data_x, one_hot_m))
                outputs_meta /= outputs_meta.sum(dim=1, keepdim=True)
                outputs_defer = F.sigmoid(self.model_classifier(
                                          data_x)[:, n_classes])
