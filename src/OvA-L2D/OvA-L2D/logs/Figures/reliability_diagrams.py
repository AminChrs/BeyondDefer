...
def compute_calibration_(logits, labels, n_classes, n_bins=20):
	num_bins = n_bins
	...
	confidences, predictions = torch.max(softmaxes[:,:n_classes]/torch.sum(softmaxes[:,:n_classes], dim=1, keepdim=True), 1)
	accuracies = predictions.eq(labels)
	print("Accuracy: {}".format(accuracies.float().mean()))
...
def _reliability_diagram_subplot(bin_data, figsize, dpi,
								 draw_ece=True, 
								 draw_bin_importance=False,
								 title="Reliability Diagram", 
								 xlabel="Confidence", 
								 ylabel=r"Expected accuracy"):
	...
	gap_plt = ax.bar(positions, np.abs(accuracies - confidences), 
					 ...
					 edgecolor=colors, color=colors, label="Gap", linewidth=1)

	acc_plt = ax.bar(positions, 0, bottom=accuracies, width=widths,
					 edgecolor="black", color="black", alpha=1.0,
					 label="Accuracy", linewidth=3)
	...
	ax.legend(handles=[gap_plt, acc_plt], prop={'size':12, 'weight':'bold'}, loc='lower left', mode='expand', ncol=2, bbox_to_anchor=(0,1.02,1,0.2))
	ax.grid()
	plt.tight_layout()
	plt.show()
	return fig

def _confidence_histogram_subplot(bin_data, figsize, dpi,
								  draw_averages=True,
	...
	ax.bar(positions, counts, width=bin_size * 0.9, alpha=0.5)
	...
	if draw_averages:
		...
		ax.legend(handles=[acc_plt, conf_plt], prop={'size':12, 'weight':'bold'})
	plt.tight_layout()
	plt.show()
	return fig
...
def _reliability_diagram_combined(bin_data, 
	...
	fig1 = _reliability_diagram_subplot(bin_data, figsize, dpi, draw_ece, draw_bin_importance, 
								 title=title, xlabel=r"Confidence")

	# Draw the confidence histogram upside down.
	# orig_counts = bin_data["counts"]
	# bin_data["counts"] = bin_data["counts"]
	# fig2 = _confidence_histogram_subplot(bin_data, figsize, dpi, draw_averages, title="")
	...
	if return_fig: return fig1, None  #, fig2


def reliability_diagram(logits, labels,  n_classes, num_bins=10,
	...
	"""
	bin_data = compute_calibration_(logits, labels, n_classes, num_bins)
